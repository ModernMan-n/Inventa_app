...
type PaperKey = keyof typeof PAPER_SIZES;
        ...
        for (let r = 0; r < totalRows; r += 3) {
          ...
          for (let rr = top; rr <= barcodeRow; rr++) {
            for (let cc = leftCol; cc <= rightCol; cc++) {
              ...
              // Строка со штрих-кодом — третья строка блока
              if (rr === barcodeRow && cell.value) {
                cell.font = {
                  name: "Code 128", // имя шрифта как в системе
                  size: 24,         // 24 pt по умолчанию
                };
                cell.alignment = {
                  horizontal: "center",
                  vertical: "middle",
  ...
  <span>Для корректного отображения штрих-кодов в Excel установите шрифт </span>
  <button
    type="button"
    className="link-btn"
  ...
  >
    ...
    >
      ...
      <ol className="modal-list">
        <li>Откройте скачанный файл <b>Code128.ttf</b>.</li>
        <li>Нажмите кнопку <b>«Установить»</b> (в Windows) или добавьте шрифт в «Шрифты» (macOS).</li>
        <li>Перезапустите Excel, если он был открыт.</li>
        <li>Убедитесь, что в этикетках строка со штрих-кодом отображается полосами.</li>
      </ol>
      <button
...
function buildLabelsTwoColumns(rows: InputRow[]): (string | null)[][] {
  ...
  for (const row of rows) {
    ...
    const qty = Number(row["Количество"] ?? 1) || 1;
